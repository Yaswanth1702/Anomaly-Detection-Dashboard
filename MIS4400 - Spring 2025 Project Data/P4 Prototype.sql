...
IF OBJECT_ID('tempdb..#MainPivot') IS NOT NULL
    DROP TABLE #MainPivot;

IF OBJECT_ID('tempdb..#AccountTmp') IS NOT NULL
...
CREATE TABLE #MainPivot (
    ...
    PeriodID INT,
    AccountID INT,
    Amount DECIMAL(18,2), -- Column for amount data
    PercentCol DECIMAL(18,2),
    IsExpected TINYINT, -- New column
    IsSubmitted TINYINT, -- New column
    PRIMARY KEY CLUSTERED (
        StoreID, FranID, PeriodID, AccountID
    )
);
...
INSERT INTO #MainPivot (
    StoreID, FranID, PeriodID, AccountID, Amount, PercentCol, IsExpected, IsSubmitted
)
SELECT 
    StoreID,
    FranID,
    PeriodID,
    AccountID,
    Amount, -- Column for amount data
    PercentCol, -- Column for percentage data
...
(2, 'Threshold', '$', '[10] <= 0', 'Product Sales ([10]) is negative.', 3, 'Organization'),
(3, 'Threshold', '$', '[330] <= 0', 'Non-Product Sales ([330]) is negative.', 3, 'Organization'),
(4, 'Percentage', '%', '([20] / NULLIF([50],0)) >= 0.85', 'Food Costs ([20]) exceeds 85% of Total Costs ([50]).', 2, 'Organization'),
(5, 'Percentage', '%', '([30] / NULLIF([50],0)) <= 0.35', 'Paper Costs ([30]) does NOT exceed 35% of Total Costs ([50]).', 2, 'Organization'),
...
(11, 'Threshold', '$', '[60] <= 0', 'Gross Profit ([60]) is negative.', 2, 'Organization'),
(12, 'Threshold', '$', '[100] <= 0', 'Profit Before Taxes ([100]) is negative.', 2, 'Organization'),
(13, 'Percentage', '%', '([50] / NULLIF([10],0)) NOT BETWEEN 0.10 AND 0.50', 'The Cost of Sales ([50]) over Product Sales ([10]) ratio is greater than 50% or less than 10%.', 2, 'Organization');
